...
**`/setup` を開いている間に**ウォレットの接続先アカウントを切り替えたり、Polygon以外のネットワークへ変えたりすると、セッションは自動で破棄されます。署名したウォレットと操作しているウォレットがずれた状態を残さないためです。元のアカウントへ戻してから、もう一度サインインしてください。

他のページを開いている間に切り替えた場合、セッションは署名したときのアドレスのまま残ります（[Issue #44](https://github.com/henkaku-center/initiation/issues/44)）。ヘッダーの表示も切り替わらないので、別のアカウントで操作したいときは `/setup` でサインインし直してください。
