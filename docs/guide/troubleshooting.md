...
なお、ページを再読み込みしたときに `/setup` が「署名してサインイン」に戻るのは、セッションが切れたからとは限りません（[Issue #40](https://github.com/henkaku-center/initiation/issues/40)）。画面の表示がサーバーのセッションを見ていないためで、他のページが正常に使えていればサインインは有効なままです。
